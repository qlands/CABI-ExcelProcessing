...
public class BlockDao extends BaseDao implements IBlockDao {
  ...
  public boolean existsBlockById(String trialUid, int blockId) {
    int count = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM ofrafertrials.block WHERE trial_id = ? AND block_id = ?", Integer.class, trialUid, String.valueOf(blockId));
    return count > 0;
  }

  @Override
  public boolean existsBlock(Block block) {
    return existsBlockById(block.getTrialUniqueId(), block.getBlockNumber());
  }
